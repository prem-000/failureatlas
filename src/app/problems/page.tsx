...
function DiffViewer({ ops }: { ops: Array<{ type: string; content: string }> }) {
  ...
  return (
    <div className="diff-scroll-wrapper">
      <div style={{ fontFamily: 'monospace', fontSize: 'clamp(10px, 2.5vw, 11px)', lineHeight: 1.6 }}>
...
function ExpandedRow({ slug, submissions }: { slug: string; submissions: Submission[] }) {
  ...
  return (
    <div style={{ padding: '12px 16px 16px', background: '#111111', borderTop: '1px solid #1a1a1a' }}>
      <div style={{ fontSize: 11, color: '#52525b', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 10 }}>
        Attempt History
      </div>
        ...
        {submissions.map(sub => (
          <div key={sub.eventId}>
            ...
            >
              ...
              <span style={{ fontSize: 12, color: '#a1a1aa' }}>Attempt #{sub.attemptNumber}</span>
              <span style={{ fontSize: 11, color: '#52525b', marginLeft: 'auto' }}>
                {new Date(sub.timestamp).toLocaleString()}
              </span>
              <span style={{ color: '#52525b', fontSize: 12 }}>{selectedId === sub.eventId ? '▲' : '▼'}</span>
            </button>
            ...
            {selectedId === sub.eventId && (
              <div style={{ marginTop: 8, background: '#161616', borderRadius: 8, border: '1px solid #1f1f1f', overflow: 'hidden' }}>
                {loading ? (
                  ...
                  <>
                    <div style={{ padding: '10px 14px', borderBottom: '1px solid #1f1f1f', display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
...
// ─── Mobile Card ───────────────────────────────────────────────────────────────
function MobileProblemCard({ g, expanded, onToggle }: { g: ProblemGroup; expanded: boolean; onToggle: () => void }) {
  return (
    <div className="mobile-problem-card">
      {/* Top row: status + title */}
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
        <div style={{ marginTop: 1 }}><StatusBadge status={g.lastStatus} /></div>
        <span style={{ fontSize: 14, fontWeight: 600, color: '#f4f4f5', flex: 1, lineHeight: 1.3 }}>{g.title}</span>
      </div>

      {/* Meta row */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 12, color: '#71717a' }}>
          {g.totalAttempts} attempt{g.totalAttempts !== 1 ? 's' : ''}
        </span>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 2 }}>
        <Link
          ...
          style={{
            fontSize: 12, color: '#ff5f52', textDecoration: 'none', fontWeight: 600,
            background: '#ff5f5215', border: '1px solid #ff5f5230', borderRadius: 7,
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 7, padding: '6px 10px', color: '#71717a', cursor: 'pointer',
            fontSize: 12, display: 'flex', alignItems: 'center', gap: 4,
          }}
        >
          <span style={{ fontSize: 10 }}>{expanded ? '▲' : '▼'}</span>
          {g.submissions.length > 1 ? `${g.submissions.length} attempts` : 'History'}
        </button>
      </div>

      {expanded && <ExpandedRow slug={g.slug} submissions={g.submissions} />}
    </div>
  );
}

// ─── Desktop Row ──────────────────────────────────────────────────────────────
function DesktopProblemRow({ g, expanded, onToggle }: { g: ProblemGroup; expanded: boolean; onToggle: () => void }) {
  return (
    <div style={{ background: '#191919', borderRadius: 10, border: '1px solid #1f1f1f', overflow: 'hidden' }}>
      <button
        onClick={onToggle}
        style={{
          ...
          cursor: 'pointer', textAlign: 'left', alignItems: 'center', transition: 'background 0.15s',
        }}
      ...
      >
        ...
        <Link
          ...
          style={{
            ...
            background: '#ff5f5215', border: '1px solid #ff5f5230', borderRadius: 6, padding: '4px 10px',
          }}
        >
          Full Analysis
        </Link>
...
export default function ProblemsPage() {
    ...
    for (const sub of submissions) {
      if (!map.has(sub.problemSlug)) {
        map.set(sub.problemSlug, {
          slug: sub.problemSlug, title: sub.problemTitle, submissions: [],
          lastStatus: sub.submissionStatus, totalAttempts: 0,
        });
      }
      const g = map.get(sub.problemSlug)!;
      g.submissions.push(sub);
      g.totalAttempts++;
      if (sub.timestamp > (g.submissions[0]?.timestamp ?? 0)) g.lastStatus = sub.submissionStatus;
    }
  ...
  const filtered = useMemo(() => {
    ...
    if (sortBy === 'recent') arr.sort((a, b) => (b.submissions[0]?.timestamp ?? 0) - (a.submissions[0]?.timestamp ?? 0));
    else if (sortBy === 'attempts') arr.sort((a, b) => b.totalAttempts - a.totalAttempts);
      ...
      <div style={{ width: '100%', maxWidth: '100%', minHeight: '100vh', background: '#131313', display: 'flex', flexDirection: 'column', overflowX: 'hidden' }}>
        ...
        <div style={{ padding: isMobile ? '16px' : '20px 28px', borderBottom: '1px solid #1f1f1f', background: '#161616' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#ff5f52', boxShadow: '0 0 8px #ff5f52', flexShrink: 0 }} />
            <span style={{ fontSize: '16px', fontWeight: 700, color: '#f4f4f5', letterSpacing: '-0.02em' }}>
              Problem Tracker
            </span>
            <span className="compact" style={{ fontSize: 11, color: '#52525b', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 20, padding: '2px 10px' }}>
              {filtered.length}
            </span>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            <input
              ...
              style={{
                flex: '1 1 200px', background: '#1a1a1a', border: '1px solid #2a2a2a',
                borderRadius: 8, padding: '9px 14px', color: '#f4f4f5', fontSize: 13, outline: 'none',
                minHeight: 'unset',
              }}
            />
            <select
              ...
              style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 8, padding: '9px 12px', color: '#a1a1aa', fontSize: 13, outline: 'none', minHeight: 'unset' }}
            >
        ...
        {/* Body */}
        <div style={{ flex: 1, padding: isMobile ? '12px 12px' : '20px 28px' }}>
          {loading ? (
            ...
            <>
              {isMobile ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {paged.map(g => (
                    <MobileProblemCard
                      ...
                      onToggle={() => toggleExpanded(g.slug)}
                    />
                  ))}
                </div>
              ) : (
                /* ─── Desktop table ── */
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px 90px 100px 120px 48px', gap: 12, padding: '8px 16px', marginBottom: 6 }}>
              ...
              {/* Pagination */}
              {totalPages > 1 && (
                <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 24, alignItems: 'center', flexWrap: 'wrap' }}>
                  <button
                    onClick={() => setPage(p => Math.max(0, p - 1))}
                    disabled={page === 0}
                    style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 7, padding: '8px 16px', color: page === 0 ? '#3f3f46' : '#a1a1aa', cursor: page === 0 ? 'not-allowed' : 'pointer', fontSize: 13 }}
                  >
                    ← Prev
                  </button>
                  ...
                  <button
                    ...
                    style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 7, padding: '8px 16px', color: page >= totalPages - 1 ? '#3f3f46' : '#a1a1aa', cursor: page >= totalPages - 1 ? 'not-allowed' : 'pointer', fontSize: 13 }}
                  >
                    Next →
                  </button>
                </div>

...
export function BehaviorInsightPanel({ weaknessId, weaknessName, isOpen, onClose }: Props) {
  ...
  const tabBar = (
    <div className="snap-x-container" style={{ display: 'flex', borderBottom: '1px solid #1f1f1f', background: '#111' }}>
      {TABS.map(tab => (
        <button
          key={tab.id}
          onClick={() => setActiveTab(tab.id)}
          className="compact snap-x-item"
          style={{
            flex: '1 0 auto', padding: '12px 16px', background: 'none', border: 'none', cursor: 'pointer',
    ...
    <>
      {loading && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center', paddingTop: 48 }}>
          <div style={{ width: 28, height: 28, border: '3px solid #2a2a2a', borderTopColor: '#ff5f52', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
  ...
  const header = (
    <div style={{ padding: '18px 20px', borderBottom: '1px solid #1f1f1f', display: 'flex', alignItems: 'flex-start', gap: 12, background: '#111' }}>
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          <div style={{ width: 3, height: 16, background: '#ff5f52', borderRadius: 2 }} />
          <span style={{ fontSize: 11, fontWeight: 700, color: '#ff5f52', letterSpacing: '0.08em' }}>BEHAVIOR INTELLIGENCE</span>
        </div>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#f4f4f5', letterSpacing: '-0.02em' }}>{weaknessName}</div>
        <div style={{ fontSize: 11, color: '#52525b', marginTop: 2 }}>Why this pattern appears · What drives it · How to fix it</div>
      </div>
      <button
        onClick={onClose}
        className="compact"
        style={{
          width: 32, height: 32, borderRadius: 8, background: '#1f1f1f', border: '1px solid #2a2a2a',
        ...
        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#2a2a2a'; (e.currentTarget as HTMLElement).style.color = '#e4e4e7'; }}
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = '#1f1f1f'; (e.currentTarget as HTMLElement).style.color = '#71717a'; }}
